...
void Output::DrawSqu(Point P1, GfxInfo RectGfxInfo, bool selected) const
{
	...
	pWind->DrawRectangle((P1.x)-(size/2), (P1.y)-(size/2), (P1.x)+(size/2), (P1.y)+(size/2), style);
...
void Output::DrawHex(Point P1, GfxInfo RectGfxInfo, bool selected) const
{
	...
	int arrY[6];
	{
		arrX[0]=P1.x-.5*size;     arrY[0]=P1.y-.5*size;
		arrX[1]=P1.x+.5*size;     arrY[1]=P1.y-.5*size;
		arrX[2]=P1.x+size;     arrY[2]=P1.y;
		arrX[3]=P1.x+.5*size;     arrY[3]=P1.y+.5*size;
		arrX[4]=P1.x-.5*size;     arrY[4]=P1.y+.5*size;
		arrX[5]=P1.x-size;     arrY[5]=P1.y;
	}
	color DrawingClr;
	if(selected)	
		DrawingClr = UI.HighlightColor; //Figure should be drawn highlighted
